# nf-core/rnaseqpca: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/rnaseqpca/usage](https://nf-co.re/rnaseqpca/usage)
...
```console
nextflow run nf-core/rnaseqpca --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/rnaseqpca
```
...
First, go to the [nf-core/rnaseqpca releases page](https://github.com/nf-core/rnaseqpca/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
